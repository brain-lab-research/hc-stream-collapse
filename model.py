...
class GPTConfig:
    ...
    expand_lss_init: Optional[float] = None
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        expand_kwargs = {}
        if getattr(config, 'expand_lss_init', None) is not None:
            expand_kwargs['lss_init'] = config.expand_lss_init
        init_hc, expand_stream, reduce_stream = hyper_conn_init_func(
            config.hyper_conn_type,
            config.hyper_conn_n,
            dim=config.n_embd,
            expand_kwargs=expand_kwargs or None,
        )
        self.expand_stream = expand_stream
        self.reduce_stream = reduce_stream
