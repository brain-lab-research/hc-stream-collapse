...
def get_expand_reduce_stream_functions(
    ...
    expand_kwargs = expand_kwargs or {}

    if expand_style == "lss":
        assert exists(dim), "dim required for expand_style='lss'"
        expand_fn = StreamExpandLSS(num_streams, dim, **expand_kwargs)
    elif add_stream_embed:
...
# expand to streams by multiplying with per-stream vector LSS (before layer 0)

class StreamExpandLSS(Module):
    """Expand to streams by multiplying with per-stream vector LSS instead of duplicating.
    For each stream s: out_s = x * lss[s] (element-wise). Init: lss_init + Xavier uniform noise."""
    def __init__(self, num_streams, dim, channel_first=False, lss_init=0.6):
        super().__init__()
        self.num_streams = num_streams
        self.channel_first = channel_first
        self.lss = nn.Parameter(torch.empty(num_streams, dim))
        nn.init.xavier_uniform_(self.lss)
        with torch.no_grad():
            self.lss.add_(lss_init)

    def forward(self, x):
        if self.channel_first:
            x = rearrange(x, 'b d ... -> b ... d')
        x = repeat(x, 'b ... d -> b ... s d', s=self.num_streams) * self.lss
        x = rearrange(x, 'b ... s d -> (b s) ... d', s=self.num_streams)
        if self.channel_first:
            x = rearrange(x, '(b s) ... d -> (b s) d ...', s=self.num_streams)
        return x
