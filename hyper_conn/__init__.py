...
def hyper_conn_init_func(hyper_conn_type: str, hyper_conn_n: int, dim=None, expand_kwargs=None):
    global flag
    if not flag:
    ...
    elif hyper_conn_type == "mhc_lite_lss":
        return mhclite_get_init_and_expand_reduce_stream_functions(
            hyper_conn_n, dim=dim, expand_style="lss", expand_kwargs=expand_kwargs or {}
        )
