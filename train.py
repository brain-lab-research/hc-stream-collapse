...
from eval_benchmarks import available_benchmarks, evaluate_benchmarks, loss_to_ppl
...
# ----- hyper conn start -----
hyper_conn_type = "none" # none, hc, mhc, mhc_lite, mhc_lite_lss
hyper_conn_n = 1 # num_streams
expand_lss_init = 0.6  # for mhc_lite_lss: LSS init base (Xavier + add), e.g. 0.6 or 1.0
...
log_interval = 1
eval_iters = 200
eval_benchmarks = True # WikiText-103 / C4 if data/*.bin exist
eval_benchmark_iters = 200
...
if hyper_conn_type == "mhc_lite_lss":
    out_prefix_method = f"{out_prefix_method}-lss_init_{expand_lss_init}"
...
model_args = dict(
    ...
    expand_lss_init=expand_lss_init if hyper_conn_type == "mhc_lite_lss" else None,
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        benchmark_losses = {}
        if eval_benchmarks:
            benchmark_paths = available_benchmarks()
            if benchmark_paths:
                benchmark_losses = evaluate_benchmarks(
                    model,
                    eval_iters=eval_benchmark_iters,
                ...
                for name, bench_loss in benchmark_losses.items():
                    desc_parts.append(f"{name} ppl: {loss_to_ppl(bench_loss):.2f}")
        ...
        if wandb_log:
            log_dict = {
                ...
                "val/loss": losses['val'],
                "val/ppl": loss_to_ppl(losses['val']),
                "lr": lr,
            ...
            for name, bench_loss in benchmark_losses.items():
                log_dict[f"benchmark/{name}_loss"] = bench_loss
                log_dict[f"benchmark/{name}_ppl"] = loss_to_ppl(bench_loss)
